...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", type=str,
        default="./eval_outputs/mcgdf_episode_animation_heavy.mp4",
    ...
    parser.add_argument(
        "--checkpoint", type=str, 
        default="/home/sungkwon/IsaacLab-RE0409/IsaacLab-main/scripts/world_model/Physics/Robot_and_Object/Contact_Flag_discrete/outputs_mcgdf/run_20260610_234838/best.pt",
    ...
    parser.add_argument(
        "--override_contact_flag_distance", action="store_true", default=True,
        help="Bypass the learned ContactFlagHead at inference and set "
             "c_t = 1 iff ||p_ee(q_t) - p_o_t|| < "
             "--contact_flag_distance_threshold.  The forward pass uses "
             "the deterministic distance rule instead of the network.",
    )
    parser.add_argument(
        "--contact_flag_distance_threshold", type=float, default=0.05,
        help="Threshold (meters) used when --override_contact_flag_distance "
             "is set.  c_t = 1 iff gripper-to-object distance < threshold.",
    )
    parser.add_argument(
    ...
    # ----- Ground-truth future trajectory overlay.  At every frame ``t``
    # the script draws the recorded gripper-tip and cube-center positions
    # for the next ``--pred_horizon`` steps (truncated near the end of
    # the episode), drawn as SOLID lines that visually pair with the
    # DASHED prediction lines above.  Defaults are on -- pass
    # --no_gt_future to disable.
    parser.add_argument(
        "--show_gt_future", action="store_true", default=True,
        help="Draw the recorded next-N-step gripper-tip and cube-center "
             "trajectories.  N is taken from --pred_horizon.",
    )
    parser.add_argument(
        "--no_gt_future", dest="show_gt_future", action="store_false",
        help="Disable the GT future trajectory overlay.",
    )
    parser.add_argument(
        "--gt_future_gripper_color", type=str, default="tab:cyan",
        help="Color of the GT future gripper-tip trajectory line.",
    )
    parser.add_argument(
        "--gt_future_cube_color", type=str, default="tab:green",
        help="Color of the GT future cube-center trajectory line.",
    )
    parser.add_argument(
        "--gt_future_linewidth", type=float, default=2.0,
        help="Linewidth of the GT future trajectory lines.",
    )
    # ----- Signal panel.  Adds a time-series plot to the right of the 3-D
    # animation showing the model's per-frame contact wrench F_c and the
    # coupling factor alpha as the animation advances.  Requires
    # --checkpoint so the per-frame model outputs exist.
    parser.add_argument(
        "--show_signal_panel", action="store_true", default=True,
        help="Add a right-side time-series panel showing F_c and alpha "
             "(requires --checkpoint).",
    )
    parser.add_argument(
        "--fc_plot_mode", type=str, default="magnitude",
        choices=["magnitude", "components"],
        help="How to draw F_c in the signal panel.  'magnitude' shows "
             "||F_c^lin|| and ||F_c^ang|| (2 lines).  'components' shows "
             "all 6 entries of F_c.",
    )
    parser.add_argument(
        "--signal_linewidth", type=float, default=1.5,
        help="Linewidth of the signal-panel lines.",
    )
...
class DistanceThresholdContactFlagHead(torch.nn.Module):
    """Drop-in replacement for ``ContactFlagHead`` used at inference only.

    Returns ``c_t = 1`` iff ``||displacement||_2 < threshold``.  The
    forward signature matches ``ContactFlagHead.forward`` so the host
    model can swap this in without changing any call sites:

      ``c_hard, c_prob, logits = head(displacement, object_lin_vel, q_dot, z)``

    ``object_lin_vel``, ``q_dot``, and ``z`` are accepted but ignored.
    The returned ``c_prob`` equals the hard 0/1 decision, and ``logits``
    saturate to +/-10 so downstream code that inspects ``sigmoid(logits)``
    sees a consistent (1 or 0) probability.  No learnable parameters --
    this module is not intended for training.
    """

    def __init__(self, threshold: float) -> None:
        super().__init__()
        self.threshold = float(threshold)
        # Optional FK module + per-episode relative-to-absolute joint offset.
        # When both are set, ``forward`` recomputes the displacement in
        # the *physical-meters* world frame (the model's internal
        # ``self.fk(q)`` uses relative joint angles and therefore lives
        # in a non-physical mixed frame).
        self._fk_module: FrankaForwardKinematics | None = None
        self.register_buffer(
            "_rel_to_abs_offset", torch.zeros(0), persistent=False,
        )
        # Running min/max of ||displacement|| (in the frame actually used
        # for thresholding) seen so far, for calibration.  ``inf`` /
        # ``-inf`` sentinels make the first observation unconditionally
        # write.  Use ``reset_stats()`` between episodes if you want to
        # accumulate per-episode rather than across the whole run.
        self.register_buffer(
            "_dist_min", torch.tensor(float("inf")), persistent=False,
        )
        self.register_buffer(
            "_dist_max", torch.tensor(float("-inf")), persistent=False,
        )
        self.register_buffer(
            "_n_calls", torch.tensor(0, dtype=torch.long), persistent=False,
        )
        self.register_buffer(
            "_n_hits", torch.tensor(0, dtype=torch.long), persistent=False,
        )

    def set_fk(self, fk_module: FrankaForwardKinematics) -> None:
        """Attach an FK module so the head can recompute distance in the
        absolute-joint-angle frame.  Idempotent."""
        self._fk_module = fk_module

    def set_rel_to_abs_offset(self, offset: torch.Tensor) -> None:
        """Provide the per-episode ``q_abs - q_rel`` offset (length =
        ``robot_dof``).  Must be on the same device as the FK module."""
        self._rel_to_abs_offset = offset.detach().to(
            device=self._dist_min.device,
            dtype=torch.float32,
        )

    @property
    def physical_frame_active(self) -> bool:
        """True iff both the FK and the per-episode offset are set."""
        return (
            self._fk_module is not None
            and isinstance(self._rel_to_abs_offset, torch.Tensor)
            and self._rel_to_abs_offset.numel() > 0
        )

    def reset_stats(self) -> None:
        self._dist_min.fill_(float("inf"))
        self._dist_max.fill_(float("-inf"))
        self._n_calls.zero_()
        self._n_hits.zero_()

    def stats(self) -> dict[str, float]:
        n = int(self._n_calls.item())
        return {
            "dist_min_m": float(self._dist_min.item()) if n > 0 else float("nan"),
            "dist_max_m": float(self._dist_max.item()) if n > 0 else float("nan"),
            "n_calls": n,
            "n_hits": int(self._n_hits.item()),
            "hit_frac": float(self._n_hits.item()) / max(1, n),
    ...
    def forward(
        ...
        object_lin_vel: torch.Tensor,  # noqa: ARG002 -- kept for signature parity
        q_dot: torch.Tensor,           # noqa: ARG002 -- kept for signature parity
        z: torch.Tensor | None,         # noqa: ARG002 -- kept for signature parity
        q: torch.Tensor | None = None,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        # The model's internal ``self.fk(q)`` uses relative joint
        # angles, so ``displacement`` arrives in a non-physical mixed
        # frame.  When we have both the FK module and the per-episode
        # ``q_abs - q_rel`` offset, we redo the FK with absolute angles
        # to put the distance back into world-frame meters.  Algebra:
        #   displacement_in       = p_ee_rel - object_pos
        #   displacement_physical = p_ee_abs - object_pos
        #                         = p_ee_abs - p_ee_rel + displacement_in
        if self.physical_frame_active and q is not None:
            offset = self._rel_to_abs_offset.to(q.device, dtype=q.dtype)
            q_abs = q + offset.unsqueeze(0)
            p_ee_abs, _ = self._fk_module(q_abs)
            p_ee_rel, _ = self._fk_module(q)
            displacement_eff = p_ee_abs - p_ee_rel + displacement
        else:
            displacement_eff = displacement
        dist = displacement_eff.norm(dim=-1, keepdim=True)
        c_hard = (dist < self.threshold).to(displacement.dtype)
        c_prob = c_hard
        logits = torch.where(
            c_hard > 0.5,
        ...
        # Bookkeeping for threshold calibration.  Detached so it stays out
        # of any autograd graph (we run under ``inference_mode`` anyway).
        with torch.no_grad():
            self._dist_min.copy_(torch.minimum(self._dist_min, dist.min().detach()))
            self._dist_max.copy_(torch.maximum(self._dist_max, dist.max().detach()))
            self._n_calls.add_(dist.numel())
            self._n_hits.add_(int(c_hard.sum().item()))
        return c_hard, c_prob, logits


def _maybe_override_contact_flag_head(
    model: MultiStepRobotObjectMCGDFWorldModel, args: argparse.Namespace,
) -> None:
    """Swap the learned ``ContactFlagHead`` for a distance-only rule.

    No-op when ``--override_contact_flag_distance`` is not set.  Mutates
    ``model`` in place; safe to call once after loading the checkpoint.
    """
    if not getattr(args, "override_contact_flag_distance", False):
        return
    # The head is owned by the inner ``RobotObjectMCGDFStep`` module
    # (``model.dynamics``); fall back to ``model`` itself in case the
    # structure changes.
    host = None
    for candidate in (getattr(model, "dynamics", None), model):
        if candidate is not None and hasattr(candidate, "contact_flag_head"):
            host = candidate
            break
    if host is None:
        raise RuntimeError(
            "No ``contact_flag_head`` attribute found on the model or its "
            "``dynamics`` submodule -- cannot install the distance-threshold "
            "override.",
        )
    threshold = float(args.contact_flag_distance_threshold)
    if threshold <= 0.0:
        raise ValueError(
            "--contact_flag_distance_threshold must be > 0; "
            f"got {threshold}.",
        )
    device = next(model.parameters()).device
    override_head = DistanceThresholdContactFlagHead(threshold).to(device)
    # Wire the model's own FK so the override can recompute distance in
    # the absolute-joint-angle (physical meters) frame.  The per-episode
    # ``rel_to_abs_offset`` is pushed in by ``precompute_pred_trajectories``
    # right before each rollout.
    host_fk = getattr(host, "fk", None)
    if host_fk is not None:
        override_head.set_fk(host_fk)
    host.contact_flag_head = override_head
    host.contact_flag_head.eval()
    print(
        f"[contact-flag override] using distance rule  |  threshold = "
        f"{threshold:.4f} m  (learned ContactFlagHead bypassed)",
    )
...
def load_checkpoint_model(
    path: str,
    device: torch.device,
    skip_contact_flag_head: bool = False,
):
    ...
    ``skip_contact_flag_head`` drops any ``contact_flag_head`` keys from
    the saved ``state_dict`` and loads with ``strict=False``.  Use it
    when ``--override_contact_flag_distance`` is set so old checkpoints
    -- whose ``ContactFlagHead`` first linear layer has the legacy
    ``1 + 3 + z`` input width -- still load even though the current
    module expects ``3 + 3 + robot_dof + z``.  The head is going to be
    swapped for ``DistanceThresholdContactFlagHead`` immediately after
    the load anyway, so its weights are irrelevant.
    """
    ...
    model = MultiStepRobotObjectMCGDFWorldModel(
        ...
        omit_object_context=bool(cfg.get("omit_object_context", True)),
        contact_flag_hidden_dim=int(cfg.get("contact_flag_hidden_dim", 64)),
        contact_flag_depth=int(cfg.get("contact_flag_depth", 2)),
        filter_steps=int(cfg.get("filter_steps", 0)),
        latent_update_hidden_dim=int(cfg.get("latent_update_hidden_dim", 64)),
        latent_update_depth=int(cfg.get("latent_update_depth", 1)),
    ).to(device)
    state_dict = ckpt["model_state_dict"]
    if skip_contact_flag_head:
        dropped = [k for k in state_dict if "contact_flag_head" in k]
        if dropped:
            state_dict = {k: v for k, v in state_dict.items() if k not in dropped}
            print(
                f"[load_checkpoint_model] dropping {len(dropped)} "
                f"contact_flag_head key(s) from state_dict "
                f"(override active; head will be replaced).",
            )
...
def precompute_pred_trajectories(
    ...
    steps and return:
      ...
      * per-frame predicted gripper-tip positions (list of (n_steps, 3) or None),
      * per-frame predicted cube-center positions (same shape),
      * per-frame contact wrench ``F_c`` taken from the *first* step of each
        rollout, shape ``(T, 6)`` with NaN where prediction is unavailable,
      * per-frame coupling factor ``alpha`` from the *first* step,
        shape ``(T, alpha_components)`` with NaN where unavailable,
      * a dict of the learned passive-decay scalars
        ``{"rho_v": float, "rho_omega": float}`` (constant per episode --
        read directly from the model's parameters).

    Element ``t`` of each list is ``None`` when prediction is not possible at
    ``t`` (no full history window yet, or no future steps left).
    """
    ...
    # If the distance-rule override is installed, push the per-episode
    # offset so its FK can compute distance in physical meters.
    override_head_attr = getattr(model.dynamics, "contact_flag_head", None)
    if isinstance(override_head_attr, DistanceThresholdContactFlagHead):
        override_head_attr.set_rel_to_abs_offset(
            torch.from_numpy(rel_to_abs_offset),
        )
    ...
    # Per-frame model outputs: F_c (6-D) and alpha (1- or 2-D depending on
    # coupling_components).  Filled at h==0 of each rollout; NaN-padded
    # where prediction is unavailable.
    n_alpha = int(getattr(model.dynamics, "coupling_components", 1))
    fc_per_t = np.full((T, 6), np.nan, dtype=np.float32)
    alpha_per_t = np.full((T, n_alpha), np.nan, dtype=np.float32)
    ...
    with torch.inference_mode():
        for t in range(base_t, T - 1):
            ...
            # Closed-loop filter window before the open-loop rollout.
            eff_filter = int(max(0, filter_steps))
            if eff_filter > 0 and z is not None and model.latent_update is not None:
                available = history_states.shape[1] - 1
                use_l = min(eff_filter, available)
                start_k = available - use_l
                for k in range(start_k, available):
                    z, _aux_k = model.step_filter(
                        history_states[:, k],
                        history_torques[:, k],
                        history_states[:, k + 1],
                        damping, friction, object_context_t, z,
                    )
            ...
            for h in range(n_steps):
                ...
                if not torch.isfinite(state).all():
                    break
                # Capture per-frame signals from the first step of each
                # rollout: F_c(s_t, tau_t, z) and alpha(s_t, ...).
                if h == 0:
                    fc_now = aux_step["contact_wrench"][0].detach().cpu().numpy()
                    fc_per_t[t] = fc_now.astype(np.float32, copy=False)
                    alpha_now = aux_step["alpha"][0].detach().cpu().numpy()
                    alpha_now = np.atleast_1d(alpha_now).reshape(-1)[:n_alpha]
                    alpha_per_t[t, : alpha_now.shape[0]] = alpha_now.astype(np.float32)
    ...
    # Read the learned passive decays directly from the model's parameters.
    # They are time-invariant within an episode (logit-stored, sigmoid-read).
    rho_consts: dict[str, float] = {"rho_v": float("nan"), "rho_omega": float("nan")}
    if hasattr(model.dynamics, "rho_v_raw"):
        rho_consts["rho_v"] = float(torch.sigmoid(model.dynamics.rho_v_raw).item())
    if hasattr(model.dynamics, "rho_omega_raw"):
        rho_consts["rho_omega"] = float(torch.sigmoid(model.dynamics.rho_omega_raw).item())
    return pred_gripper_per_t, pred_cube_per_t, fc_per_t, alpha_per_t, rho_consts
...
def _precompute_gt_future_trajectories(
    ...
    """Slice the recorded next-``horizon``-step trajectories at every frame.

    Returns two lists of length ``T``.  Element ``t`` is an
    ``(n_steps, 3)`` float array of recorded positions for
    ``t+1, t+2, ..., t+n_steps`` where ``n_steps <= horizon`` (truncated
    near the end of the episode), or ``None`` when there is no future
    frame left.  Mirrors the shape of ``precompute_pred_trajectories``'s
    return value so the two can be drawn side by side.
    """
    gt_gripper: list[np.ndarray | None] = [None] * T
    gt_cube: list[np.ndarray | None] = [None] * T
    if horizon <= 0:
        return gt_gripper, gt_cube
    gripper_xyz = joints_3d[:, -1, :]  # (T, 3) — last joint is the gripper tip
    for t in range(T - 1):
...
def render_animation(
    ...
    fc_per_t: np.ndarray | None = None,
    alpha_per_t: np.ndarray | None = None,
    rho_consts: dict[str, float] | None = None,
) -> str:
    ...
    if show_gt_future:
        ...
        gt_cube_per_t = None
    # Signal panel: only when --show_signal_panel was set AND the per-frame
    # arrays are available (i.e. a checkpoint was loaded).
    show_signal_panel = (
        bool(getattr(args, "show_signal_panel", False))
        and fc_per_t is not None
        and alpha_per_t is not None
    )
    ...
    if show_signal_panel:
        ...
        ax_fc = fig.add_subplot(gs_right[0])
        ax_alpha = fig.add_subplot(gs_right[1], sharex=ax_fc)
    else:
    ...
    # and GT future-trajectory lines (drawn whenever --show_gt_future is on).
    # A length-1 NaN segment hides each line until valid data is set.
    ...
    # Signal-panel artists: time-series of F_c and alpha.  Only constructed
    # when ``show_signal_panel`` is on (which implies fc_per_t / alpha_per_t
    # are available).
    fc_lines: list = []
    alpha_lines: list = []
    time_axis_sec: np.ndarray | None = None
    fc_mode = getattr(args, "fc_plot_mode", "magnitude")
    sig_lw = float(getattr(args, "signal_linewidth", 1.5))
    if show_signal_panel:
        time_axis_sec = np.arange(T, dtype=np.float32) * float(args.dt)
        # F_c subplot
        if fc_mode == "magnitude":
            fc_lin_mag = np.linalg.norm(fc_per_t[:, :3], axis=1)
            fc_ang_mag = np.linalg.norm(fc_per_t[:, 3:], axis=1)
            ln_lin, = ax_fc.plot([], [], color="tab:blue",
                                 linewidth=sig_lw, label=r"$\|F_c^{lin}\|$")
            ln_ang, = ax_fc.plot([], [], color="tab:red",
                                 linewidth=sig_lw, label=r"$\|F_c^{ang}\|$")
            fc_lines = [(ln_lin, fc_lin_mag), (ln_ang, fc_ang_mag)]
            y_top = float(np.nanmax(
                np.concatenate([fc_lin_mag, fc_ang_mag])
            )) if np.any(np.isfinite(fc_per_t)) else 1.0
            y_top = max(y_top * 1.15, 1e-3)
            ax_fc.set_ylim(0.0, y_top)
        else:  # "components"
            comp_labels = [r"$F_c^{lin,x}$", r"$F_c^{lin,y}$", r"$F_c^{lin,z}$",
                           r"$F_c^{ang,x}$", r"$F_c^{ang,y}$", r"$F_c^{ang,z}$"]
            comp_colors = ["tab:blue", "tab:orange", "tab:green",
                           "tab:red",  "tab:purple", "tab:brown"]
            for i in range(6):
                ln_i, = ax_fc.plot([], [], color=comp_colors[i],
                                   linewidth=sig_lw, label=comp_labels[i])
                fc_lines.append((ln_i, fc_per_t[:, i]))
            finite = fc_per_t[np.isfinite(fc_per_t)]
            if finite.size > 0:
                y_min = float(finite.min()) * 1.15
                y_max = float(finite.max()) * 1.15
                if y_max <= y_min:
                    y_max = y_min + 1.0
                ax_fc.set_ylim(y_min, y_max)
        ax_fc.set_xlim(time_axis_sec[0], time_axis_sec[-1])
        ax_fc.set_ylabel("F_c [N or N·m]")
        ax_fc.grid(True, alpha=0.3)
        ax_fc.legend(loc="upper right", fontsize=8, framealpha=0.85)
        ax_fc.set_title("Contact wrench F_c", fontsize=10)
        plt.setp(ax_fc.get_xticklabels(), visible=False)

        # alpha subplot
        n_alpha = alpha_per_t.shape[1]
        alpha_colors = ["tab:cyan", "tab:olive"][:n_alpha]
        alpha_labels = ([r"$\alpha$"] if n_alpha == 1
                        else [r"$\alpha^{rigid}$", r"$\alpha^{slide}$"])
        for i in range(n_alpha):
            ln_i, = ax_alpha.plot([], [], color=alpha_colors[i],
                                  linewidth=sig_lw, label=alpha_labels[i])
            alpha_lines.append((ln_i, alpha_per_t[:, i]))
        ax_alpha.set_xlim(time_axis_sec[0], time_axis_sec[-1])
        ax_alpha.set_ylim(-0.05, 1.05)
        ax_alpha.set_xlabel("episode time [s]")
        ax_alpha.set_ylabel(r"$\alpha,\ \rho$")
        ax_alpha.grid(True, alpha=0.3)
        # Passive-decay scalars rho_v / rho_omega: time-invariant per
        # episode, drawn as static horizontal reference lines on the same
        # subplot since they live in the same [0,1] range as alpha.
        if rho_consts is not None:
            rho_v_val = rho_consts.get("rho_v", float("nan"))
            rho_omega_val = rho_consts.get("rho_omega", float("nan"))
            if np.isfinite(rho_v_val):
                ax_alpha.axhline(
                    rho_v_val, color="tab:gray", linestyle="--",
                    linewidth=sig_lw * 0.9, alpha=0.85,
                    label=rf"$\rho_v={rho_v_val:.3f}$",
                )
            if np.isfinite(rho_omega_val):
                ax_alpha.axhline(
                    rho_omega_val, color="tab:brown", linestyle=":",
                    linewidth=sig_lw * 0.9, alpha=0.85,
                    label=rf"$\rho_\omega={rho_omega_val:.3f}$",
                )
        ax_alpha.legend(loc="upper right", fontsize=8, framealpha=0.85)
        ax_alpha.set_title("Coupling factor & passive decays", fontsize=10)
        # Vertical cursor showing the current frame, refreshed per update.
        sig_cursor_fc = ax_fc.axvline(time_axis_sec[0], color="black",
                                      linestyle=":", linewidth=1.0, alpha=0.6)
        sig_cursor_alpha = ax_alpha.axvline(time_axis_sec[0], color="black",
                                            linestyle=":", linewidth=1.0, alpha=0.6)
    else:
        sig_cursor_fc = None
        sig_cursor_alpha = None
    ...
    def update(frame: int):
        ...
        # GT future trajectory: same horizon as the prediction so they pair
        # up visually (solid GT vs dashed prediction).
        ...
        # Signal panel: extend each F_c / alpha line up to the current frame
        # and slide the vertical cursor.
        if show_signal_panel and time_axis_sec is not None:
            up_to = frame + 1
            x_now = time_axis_sec[:up_to]
            for line, y in fc_lines:
                line.set_data(x_now, y[:up_to])
            for line, y in alpha_lines:
                line.set_data(x_now, y[:up_to])
            if sig_cursor_fc is not None:
                sig_cursor_fc.set_xdata([float(time_axis_sec[frame])] * 2)
                sig_cursor_alpha.set_xdata([float(time_axis_sec[frame])] * 2)
        time_text.set_text(
...
def render_single_episode(
    ...
    if model is not None:
        ...
        # Filter steps: -1 → fall back to the value baked into the model.
        if args_ep.filter_steps < 0:
            eff_filter = int(getattr(model, "filter_steps", 0))
        else:
            eff_filter = int(args_ep.filter_steps)
        (
            pred_gripper_per_t,
            pred_cube_per_t,
            fc_per_t,
    ...
    written = render_animation(
        ...
        fc_per_t=fc_per_t,
        alpha_per_t=alpha_per_t,
        rho_consts=rho_consts,
    )
    episode_meta = {
        "name": episode["name"],
        "T": episode["T"],
...
def main() -> None:
    ...
    if args.checkpoint is not None:
        ...
        model, cfg = load_checkpoint_model(
            ckpt_path, device,
            skip_contact_flag_head=bool(
                getattr(args, "override_contact_flag_distance", False)
            ),
        )
        _maybe_override_contact_flag_head(model, args)
        history_len = int(cfg.get("history_len", 5))
    ...
    if pred_info is not None:
        ...
        for key, value in pred_info.items():
            print(f"  {key}: {value}")
        print("--------------------------------------")
    # If the distance-rule override was installed, print the running min/max
    # gripper-cube distance observed during rollouts.  Useful for calibrating
    # ``--contact_flag_distance_threshold`` -- if dist_min_m >= threshold,
    # the flag will never fire.
    if model is not None and isinstance(
        getattr(getattr(model, "dynamics", None), "contact_flag_head", None),
        DistanceThresholdContactFlagHead,
    ):
        s = model.dynamics.contact_flag_head.stats()
        print("---- Distance-rule override diagnostics ----")
        print(f"  threshold_m: {s['threshold_m']:.4f}")
        print(f"  dist_min_m:  {s['dist_min_m']:.4f}    "
              f"dist_max_m: {s['dist_max_m']:.4f}")
        print(f"  hit_frac:    {s['hit_frac']:.4f}    "
              f"(n_hits / n_calls = {s['n_hits']} / {s['n_calls']})")
        if s["hit_frac"] == 0.0 and s["dist_min_m"] >= s["threshold_m"]:
            print(
                f"  WARNING: threshold ({s['threshold_m']:.4f} m) is below "
                f"the minimum observed distance ({s['dist_min_m']:.4f} m); "
                f"the flag will never fire.  Raise --contact_flag_distance_threshold."
            )
        print("--------------------------------------------")
    print(f"Saved: {written}")
