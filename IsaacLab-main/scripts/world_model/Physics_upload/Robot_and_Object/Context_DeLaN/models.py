...
class DerivativeMLP(nn.Sequential):
    """MLP that also propagates output derivatives with respect to its input."""
...
class MLPContextEncoder(nn.Module):
    """Flattened-history context encoder, useful as a Ver_CaDM-style ablation."""
    ...
    def __init__(
        ...
        self.net = mlp(history_len * (state_dim + torque_dim), hidden_dim, latent_dim, depth=depth)

    def forward(self, history_states: torch.Tensor, history_torques: torch.Tensor) -> torch.Tensor:
        x = torch.cat([history_states, history_torques], dim=-1).flatten(start_dim=1)
        return self.net(x)


class LSTMContextEncoder(nn.Module):
    """Temporal context encoder matching the paper's online-identification idea."""
    ...
    def __init__(
        ...
        super().__init__()
        self.lstm = nn.LSTM(
            input_size=state_dim + torque_dim,
    ...
    def forward(self, history_states: torch.Tensor, history_torques: torch.Tensor) -> torch.Tensor:
        x = torch.cat([history_states, history_torques], dim=-1)
        output, _ = self.lstm(x)
        return self.proj(output[:, -1])


class ContextDeLaNStructuredTerms(nn.Module):
    """Predict robot DeLaN terms from q and latent context z."""
    ...
    def __init__(
        ...
        input_dim = robot_dof + latent_dim
        n_offdiag = robot_dof * (robot_dof - 1) // 2
        self.g_net = mlp(input_dim, hidden_dim, robot_dof, depth=2)
        self.l_diag_net = DerivativeMLP(input_dim, hidden_dim, robot_dof, depth=2)
        self.l_offdiag_net = DerivativeMLP(input_dim, hidden_dim, n_offdiag, depth=2)
    ...
    def make_l_and_derivatives(
        self, q: torch.Tensor, z: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        qz = torch.cat([q, z], dim=-1)
        batch = q.shape[0]
        l_diag_raw, dl_diag_raw_dqz = self.l_diag_net.forward_with_jacobian(qz)
        l_offdiag, dl_offdiag_dqz = self.l_offdiag_net.forward_with_jacobian(qz)
        dl_diag_raw_dq = dl_diag_raw_dqz[..., : self.robot_dof]
        dl_offdiag_dq = dl_offdiag_dqz[..., : self.robot_dof]
    ...
    def inertia(self, q: torch.Tensor, z: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        L, dL_dq, l_diag, l_offdiag = self.make_l_and_derivatives(q, z)
        H = L @ L.transpose(-1, -2)
    ...
    def forward(self, q: torch.Tensor, dq: torch.Tensor, z: torch.Tensor) -> dict[str, torch.Tensor]:
        H, L, dL_dq, l_diag, l_offdiag = self.inertia(q, z)
        g = self.g_net(torch.cat([q, z], dim=-1))
...
class ContextRobotObjectDeLaNStep(nn.Module):
    """One context-conditioned robot-object dynamics step."""
    ...
    def __init__(
        ...
        if torque_dim != robot_dof:
            raise ValueError("Context DeLaN expects torque_dim == robot_dof.")
        self.robot_dof = robot_dof
        ...
        self.dt = dt
        self.structured_terms = ContextDeLaNStructuredTerms(robot_dof, latent_dim, hidden_dim, eps=inertia_eps)
        coupling_input_dim = self.state_dim + torque_dim + latent_dim
    ...
    def forward(self, state: torch.Tensor, torque: torch.Tensor, z: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        coupling_input = torch.cat([state, torque, z], dim=-1)
        ...
        terms = self.structured_terms(q, dq, z)
        H = terms["H"]
        g = terms["g"]
        ...
        # The latent model does not know object mass explicitly; use unit-mass force proxies for auxiliary supervision.
        gravity_acc = torch.zeros_like(object_lin_acc)
        gravity_acc[:, 2] = -9.81
        object_inertial_force = object_lin_acc
        object_gravity_force = gravity_acc
        ...
        aux = {
            ...
            "ddq": ddq,
            "inertial": inertial,
            "inverse_dynamics_tau": inverse_dynamics_tau,
...
class ContextRobotObjectDeLaNWorldModel(nn.Module):
    """Recursive rollout with a history-inferred latent context z."""
    ...
    def __init__(
        ...
        latent_dim: int = 16,
        hidden_dim: int = 256,
        context_encoder: str = "lstm",
        lstm_layers: int = 1,
        dt: float = 0.02,
        inertia_eps: float = 1.0e-4,
    ) -> None:
        super().__init__()
        self.spec = ContextDeLaNStateSpec(robot_dof=robot_dof, torque_dim=torque_dim, latent_dim=latent_dim)
        self.history_len = history_len
        self.context_encoder_type = context_encoder
        if context_encoder == "mlp":
            self.context_encoder = MLPContextEncoder(
                self.spec.state_dim, torque_dim, history_len, latent_dim, hidden_dim
            )
        elif context_encoder == "lstm":
            self.context_encoder = LSTMContextEncoder(
                self.spec.state_dim, torque_dim, latent_dim, hidden_dim, layers=lstm_layers
            )
        else:
            raise ValueError("context_encoder must be 'mlp' or 'lstm'.")
        self.dynamics = ContextRobotObjectDeLaNStep(
            robot_dof=robot_dof,
    ...
    def encode_context(self, history_states: torch.Tensor, history_torques: torch.Tensor) -> torch.Tensor:
        if history_states.shape[1] != self.history_len or history_torques.shape[1] != self.history_len:
            raise ValueError(f"Expected history length {self.history_len}.")
        return self.context_encoder(history_states, history_torques)
    ...
    def forward(
    ...
    ) -> torch.Tensor | tuple[torch.Tensor, list[dict[str, torch.Tensor]], torch.Tensor]:
        ...
        for h in range(future_torques.shape[1]):
            state, aux = self.dynamics(state, future_torques[:, h], z)
            preds.append(state)
            if return_aux:
                aux_list.append(aux)
        pred_future = torch.stack(preds, dim=1)
        if return_aux:
            return pred_future, aux_list, z
        return pred_future
...
def weighted_rollout_mse(
    ...
    object_lin_vel_loss = F.mse_loss(pred[..., robot_state_dim + 7 : robot_state_dim + 10], target[..., robot_state_dim + 7 : robot_state_dim + 10])
    object_ang_vel_loss = F.mse_loss(pred[..., robot_state_dim + 10 : robot_state_dim + 13], target[..., robot_state_dim + 10 : robot_state_dim + 13])
    loss = (
        q_weight * q_loss
        + dq_weight * dq_loss
...
def supervised_robot_dynamics_loss(
    ...
    if not aux_list:
        zero = future_torques.new_zeros(())
        return zero, {}
    total = aux_list[0]["H"].new_zeros(())
    ...
    terms = [
        ("mass_matrix_mse", lambda_mass_matrix, stack_aux(aux_list, "H"), future_robot_dynamics["mass_matrix"]),
        ("inertial_mse", lambda_inertial, stack_aux(aux_list, "inertial"), future_robot_dynamics["inertial"]),
        ("coriolis_gt_mse", lambda_coriolis_gt, stack_aux(aux_list, "coriolis"), future_robot_dynamics["coriolis"]),
        ("gravity_mse", lambda_gravity, stack_aux(aux_list, "g"), future_robot_dynamics["gravity"]),
        ("qdd_mse", lambda_qdd, stack_aux(aux_list, "ddq"), future_robot_dynamics["qdd"]),
        ("inverse_dynamics_mse", lambda_inverse_dynamics, stack_aux(aux_list, "inverse_dynamics_tau"), future_robot_dynamics["inverse_dynamics_tau"]),
        ("residual_torque_mse", lambda_residual_torque, pred_residual, target_residual),
    ]
    for name, weight, pred_value, target_value in terms:
        mse = F.mse_loss(pred_value, target_value)
...
def supervised_object_dynamics_loss(
    ...
    if not aux_list:
        first = next(iter(future_object_dynamics.values()))
        return first.new_zeros(()), {}
    total = aux_list[0]["object_lin_acc"].new_zeros(())
    metrics: dict[str, torch.Tensor] = {}
    terms = [
        ("object_lin_acc_mse", lambda_object_lin_acc, stack_aux(aux_list, "object_lin_acc"), future_object_dynamics["root_lin_acc_w"]),
        ("object_ang_acc_mse", lambda_object_ang_acc, stack_aux(aux_list, "object_ang_acc"), future_object_dynamics["root_ang_acc_w"]),
        ("object_external_force_mse", lambda_object_external_force, stack_aux(aux_list, "object_external_force"), future_object_dynamics["external_force_est_w"]),
    ]
    for name, weight, pred_value, target_value in terms:
        mse = F.mse_loss(pred_value, target_value)
        metrics[name] = mse.detach()
...
def auxiliary_regularization(
    ...
    lambda_context_l2: float = 0.0,
) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    if not aux_list:
        zero = z.new_zeros(())
        return zero, {"coriolis_reg": zero, "residual_torque_reg": zero, "context_l2_reg": zero}
    ...
    context_reg = z.pow(2).mean()
    total = lambda_coriolis * coriolis_reg + lambda_residual_l2 * residual_reg + lambda_context_l2 * context_reg
    return total, {
        "coriolis_reg": coriolis_reg.detach(),
        "residual_torque_reg": residual_reg.detach(),
        "context_l2_reg": context_reg.detach(),
    }
