...
from dataset import (
    ContextDeLaNRolloutDataset,
    ContextDeLaNStateLayout,
    discover_hdf5_files,
    load_all_episode_refs,
    split_episode_refs,
)
from models import (
    ContextRobotObjectDeLaNWorldModel,
...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(description="Train Context DeLaN robot-object world model.")
    parser.add_argument("--dataset_dir", type=str, default="./datasets")
    parser.add_argument("--dataset_file", type=str, default=None)
    parser.add_argument("--history_len", type=int, default=5, help="K+1 states and torques used by the context encoder.")
    ...
    parser.add_argument("--context_encoder", type=str, default="lstm", choices=["mlp", "lstm"])
    ...
    parser.add_argument("--filter_pre_contact", action="store_true", default=False)
...
def make_dataloaders(cfg: TrainConfig) -> tuple[DataLoader, DataLoader, ContextDeLaNStateLayout, dict[str, object]]:
    ...
    train_dataset = ContextDeLaNRolloutDataset(episode_refs=train_refs, **kwargs)
    val_dataset = ContextDeLaNRolloutDataset(episode_refs=val_refs, **kwargs)
...
def run_epoch(
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        with torch.set_grad_enabled(is_train):
            pred_future, aux_list, z = model(history_states, history_torques, future_torques, return_aux=True)
            ...
            reg_loss, regs = auxiliary_regularization(
                ...
                lambda_context_l2=cfg.lambda_context_l2,
            )
        ...
        totals["context_abs_mean"] += float(z.abs().mean().item())
...
def main() -> None:
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        print(
            ...
            f"val_context_abs={val_metrics['context_abs_mean']:.6f}"
        )
