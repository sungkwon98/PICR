...
class TrainConfig:
    ...
    contact_depth: int
    # MCGDF_object_mlp variant: sizes of the next-state ObjectStateMLP head.
    object_mlp_hidden_dim: int
    object_mlp_depth: int
    tool_z_offset: float
    dt: float
    batch_size: int
    epochs: int
...
def parse_args() -> TrainConfig:
    ...
    parser.add_argument(
        "--object_mlp_hidden_dim", type=int, default=256,
        help="Hidden width of the ObjectStateMLP next-state head.",
    )
    parser.add_argument(
        "--object_mlp_depth", type=int, default=3,
        help="Number of FiLM blocks in the ObjectStateMLP next-state head.",
    )
...
def main() -> None:
    ...
    model = MultiStepRobotObjectMCGDFWorldModel(
        ...
        object_mlp_hidden_dim=cfg.object_mlp_hidden_dim,
        object_mlp_depth=cfg.object_mlp_depth,
        dt=cfg.dt,
        friction_eps=cfg.friction_eps,
