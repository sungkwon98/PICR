...
# ----------------------------------------------------------------------------
# Coupling head + latent innovation update (No-Object-Euler architecture)
# ----------------------------------------------------------------------------

class CouplingHead(nn.Module):
    """Compute the contact-gated coupling factor alpha_t in [0, 1].

    Inputs at step t:
      - ||F_c^lin||_2 (1)     -- magnitude of the predicted linear contact force
      - displacement d_t (3)  -- p_gripper(q_t) - p_object_t in world frame
      - latent z (latent_dim) -- optional context latent (omitted when None)

    Output: alpha_t (B, alpha_components) in [0, 1] via sigmoid.

    ``alpha_components=1`` is the single "rigid" coupling; pass 2 for
    (rigid, slide) two-component gating.
    """
    ...
    def __init__(
        ...
        for _ in range(depth):
            layers += [nn.Linear(last, hidden_dim), nn.SiLU()]
            last = hidden_dim
        layers.append(nn.Linear(last, alpha_components))
        self.net = nn.Sequential(*layers)
        self.alpha_components = int(alpha_components)

    def forward(
        self,
        F_c_lin: torch.Tensor,
        displacement: torch.Tensor,
        z: torch.Tensor | None,
    ) -> torch.Tensor:
        f_norm = F_c_lin.norm(dim=-1, keepdim=True)
        parts = [f_norm, displacement]
...
class RobotObjectMCGDFStep(nn.Module):
    """One step of the robot+coupling-based-object world model.

    Robot side: unchanged from the original MCGDF (DeLaN H/c/g, residual
    head r_theta, joint damping/friction, and the contact reaction torque
    -J_c^T F_c that flows from Newton's third law).

    Object side (changed): no Newton-Euler integrator.  The object twist
    is produced by blending the analytic gripper twist J(q) qdot into the
    previous object twist via a contact-gated coupling factor alpha_t
    (Eqs. (future-coupling-alpha)-(future-robj) of the description).
    No m_o, I_b or mu_o appears in the forward pass: object_context is
    ...
    def __init__(
        ...
        contact_depth: int = 3,
        coupling_hidden_dim: int = 64,
        coupling_depth: int = 2,
        coupling_components: int = 1,
        init_rho_v: float = 0.95,
        init_rho_omega: float = 0.95,
        dt: float = 0.02,
        ...
        # ``omit_object_context=True`` strips the GT xi from the contact
        # head's input.  Symmetric across train and eval.
        self.omit_object_context = bool(omit_object_context)
        self.object_context_dim = 0 if self.omit_object_context else int(object_context_dim)
        ...
        self.coupling_components = int(coupling_components)
        ...
        self.contact_head = ContactWrenchHead(
            ...
            context_dim=self.object_context_dim,
            hidden_dim=contact_hidden_dim, depth=contact_depth,
            latent_dim=latent_dim,
        )
        # Coupling head: alpha_t = sigma(MLP(||F_c^lin||, displacement, z))
        self.coupling_head = CouplingHead(
            latent_dim=latent_dim,
            hidden_dim=coupling_hidden_dim,
            depth=coupling_depth,
            alpha_components=self.coupling_components,
        )
        # Passive decays rho_v, rho_omega in (0, 1].  Parameterised via
        # logit so they stay bounded and start near 1 (small dissipation).
        def _logit(p: float) -> float:
            p = float(max(min(p, 1.0 - 1e-6), 1e-6))
            return math.log(p / (1.0 - p))
        self.rho_v_raw = nn.Parameter(torch.tensor(_logit(init_rho_v), dtype=torch.float32))
        self.rho_omega_raw = nn.Parameter(torch.tensor(_logit(init_rho_omega), dtype=torch.float32))
    ...
    def coupling_object_dynamics(
        ...
        """Coupling-based object dynamics.  Replaces Newton-Euler.

        Pipeline:
          gripper twist     : (v_g, omega_g) = J_c(q) qdot
          displacement      : d = p_ee - p_o
          coupling factor   : alpha = sigmoid(MLP(||F_c^lin||, d, z))
          new object twist  : v_o   = alpha * v_g   + (1 - alpha) * rho_v   * v_o_prev
                              omega = alpha * omega_g + (1 - alpha) * rho_w * omega_prev
        """
        # gripper twist in world frame (6, 1) -> (3, 3) split
        grip_twist = torch.einsum("bij,bj->bi", J_c, dq)
        v_g = grip_twist[:, :3]
        omega_g = grip_twist[:, 3:6]
        displacement = p_ee - object_pos
        alpha = self.coupling_head(F_c[:, :3], displacement, z)
        # alpha may be (B, 1) for the single-component case or (B, 2) for
        # the two-component (rigid, slide) variant.  In the two-component
        # case we use the first channel for linear coupling and the second
        ...
        if alpha.shape[-1] == 1:
            ...
            alpha_lin = alpha[..., :1]
            alpha_ang = alpha[..., 1:2]
        rho_v = torch.sigmoid(self.rho_v_raw)
        rho_w = torch.sigmoid(self.rho_omega_raw)
        object_lin_vel_next = alpha_lin * v_g + (1.0 - alpha_lin) * rho_v * object_lin_vel
        object_ang_vel_next = alpha_ang * omega_g + (1.0 - alpha_ang) * rho_w * object_ang_vel
        return object_lin_vel_next, object_ang_vel_next, alpha, displacement, v_g, omega_g
    ...
    def forward(
        ...
        # Robot integration (unchanged) -- needed to know q_next for FK below.
        dq_next = dq + ddq * self.dt
        q_next = q + dq_next * self.dt

        # Object side: gripper kinematics at the *current* q (semi-implicit
        # in q would use q_next; the convention here mirrors the existing
        # MCGDF semi-implicit Euler).
        ...
        (
            ...
            v_g,
            omega_g,
        ) = self.coupling_object_dynamics(
            q, dq, object_pos, object_lin_vel, object_ang_vel, F_c, J_c, p_ee, z,
        )

        # Semi-implicit Euler position update + quaternion exp-map.
        ...
        # Effective accelerations for any downstream supervision that
        # still expects ``object_lin_acc`` / ``object_ang_acc`` in aux.
        # Computed as finite differences of the predicted velocities.
        lin_acc = (object_lin_vel_next - object_lin_vel) / self.dt
        ang_acc = (object_ang_vel_next - object_ang_vel) / self.dt
        ...
        aux = {
            ...
            # Object-side: kept as effective accelerations (FD on predicted
            # velocities) so existing supervised_object_dynamics_loss still
            # produces a signal.  The model has no separate Newton-Euler
            # gravity term in the No-Object-Euler architecture; we report
            # a zero gravity slot for backward compat.
            "object_lin_acc": lin_acc, "object_ang_acc": ang_acc,
            "object_gravity_acc": torch.zeros_like(F_c[:, :3]),
            "object_inertial_force": torch.zeros_like(F_c[:, :3]),
            "object_gravity_force": torch.zeros_like(F_c[:, :3]),
            # New coupling-side fields.
            "alpha": alpha, "gripper_pos": p_ee,
            "gripper_obj_displacement": displacement,
            "gripper_lin_vel": v_g, "gripper_ang_vel": omega_g,
            "rho_v": torch.sigmoid(self.rho_v_raw).detach().expand(F_c.shape[0]),
            "rho_omega": torch.sigmoid(self.rho_omega_raw).detach().expand(F_c.shape[0]),
...
class MultiStepRobotObjectMCGDFWorldModel(nn.Module):
    ...
    def __init__(
        ...
        self.dynamics = RobotObjectMCGDFStep(
            ...
            coupling_hidden_dim=coupling_hidden_dim,
            coupling_depth=coupling_depth,
            coupling_components=coupling_components,
            init_rho_v=init_rho_v,
            init_rho_omega=init_rho_omega,
