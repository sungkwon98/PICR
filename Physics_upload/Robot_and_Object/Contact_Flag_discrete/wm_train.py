...
class TrainConfig:
    ...
    # BCE weight for the heuristic gripper-cube distance contact label.
    # 0 disables the BCE term (the flag is then learned purely via STE).
    lambda_contact_flag: float
    # Distance below which the heuristic label sets c_t = 1.
    ...
    # ---- No-Object-Euler architecture flags --------------------------
    # ``omit_object_context``: never feed GT xi into the model's forward
    # pass (default True for this folder; xi is still used by training
    # losses and supervised targets, but the model is symmetric across
    # train and eval in terms of inputs).
    omit_object_context: bool
    # Discrete contact-flag head sizes (binary c_t in {0, 1}).
    contact_flag_hidden_dim: int
    contact_flag_depth: int
    # Closed-loop filter window: L steps of one-step prediction + latent
    # innovation update before the open-loop rollout begins.
    filter_steps: int
    latent_update_hidden_dim: int
    latent_update_depth: int
    beta_kl: float
    lambda_nce: float
...
def parse_args() -> TrainConfig:
    ...
    parser.add_argument(
        "--lambda_contact_flag", type=float, default=0.1,
        help="BCE weight on c_prob vs the heuristic gripper-cube distance "
             "label.  0 disables the BCE term and falls back to pure STE.",
    )
    parser.add_argument(
        "--contact_distance_threshold", type=float, default=0.05,
        help="Per-frame label c_t_gt = 1 iff ||p_ee(q_t) - p_o_t|| < this "
             "threshold (meters).  Computed in env-local frame.",
    )
    ...
    # ----- No-Object-Euler architecture: coupling-based object dynamics
    # and online latent context update.
    parser.add_argument(
        "--omit_object_context", action="store_true", default=True,
        help="Strip GT object context (mass / inertia / friction) from the "
             "model's forward pass.  Default ON for this folder; xi is "
             "used by training losses only, not by the contact/coupling "
             "heads.",
    )
    parser.add_argument(
        "--use_object_context", dest="omit_object_context",
        action="store_false",
        help="Feed GT object context into the contact head (legacy MCGDF "
             "behaviour).  Breaks train/eval symmetry; only use for "
             "ablation against MCGDF.",
    )
    parser.add_argument(
        "--contact_flag_hidden_dim", type=int, default=64,
        help="Hidden width of the discrete contact-flag MLP.",
    )
    parser.add_argument(
        "--contact_flag_depth", type=int, default=2,
        help="Number of hidden layers in the discrete contact-flag MLP.",
    )
    parser.add_argument(
        "--filter_steps", type=int, default=0,
        help="Closed-loop filter window L_filter: number of one-step "
             "predictions + latent innovation updates run on the last "
             "L_filter transitions of the history window before the "
             "open-loop rollout.  0 disables the filter (z stays at the "
             "encoder mean).  Requires --use_context_encoder and "
             "history_len >= filter_steps + 1.",
    )
    parser.add_argument(
        "--latent_update_hidden_dim", type=int, default=64,
        help="Hidden width of the LatentInnovationUpdate cell.",
    )
    parser.add_argument(
        "--latent_update_depth", type=int, default=1,
        help="Number of hidden layers in the LatentInnovationUpdate cell.",
    )
...
def make_dataloaders(cfg: TrainConfig):
    ...
    kwargs = dict(
        ...
        contact_distance_threshold=cfg.contact_distance_threshold,
        layout=layout,
    )
...
def run_epoch(model, loader, optimizer, device, cfg: TrainConfig, max_batches):
    ...
    totals: dict[str, float] = {
        ...
        # Contact-flag BCE supervision (heuristic gripper-cube distance
        # label).  ``contact_flag_label_pos_frac`` is the fraction of
        # frames with c_gt = 1 in the batch -- handy for spotting class
        # imbalance.
        "flag_bce_loss": 0.0,
        "contact_flag_label_pos_frac": 0.0,
        "contact_flag_pred_pos_frac": 0.0,
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        future_contact_label = batch["future_contact_label"].to(device)
        ...
        with torch.set_grad_enabled(is_train):
            ...
            # ---- Contact-flag BCE supervision (heuristic distance label) ----
            # ``c_prob`` is the sigmoid of the ContactFlagHead logits and is
            # emitted per rollout step in ``aux_list``.  Stacking it along
            # the horizon axis gives shape (B, H, 1); we squeeze and BCE
            # against the (B, H) gripper-cube distance label.  Set
            # ``--lambda_contact_flag 0`` to fall back to pure STE.
            flag_prob_stack = torch.stack(
                [aux["contact_flag_prob"] for aux in aux_list], dim=1,
            ).squeeze(-1)
            flag_prob_stack = flag_prob_stack.clamp(min=1e-6, max=1.0 - 1e-6)
            flag_bce_loss = torch.nn.functional.binary_cross_entropy(
                flag_prob_stack, future_contact_label,
            )
            ...
            loss = (
                ...
                + cfg.lambda_contact_flag * flag_bce_loss
                + cfg.beta_kl * kl_loss
        ...
        for key, value in res_sup_metrics.items():
            totals[key] += float(value.item())
        totals["flag_bce_loss"] += float(flag_bce_loss.item())
        totals["contact_flag_label_pos_frac"] += float(future_contact_label.mean().item())
        totals["contact_flag_pred_pos_frac"] += float((flag_prob_stack > 0.5).float().mean().item())
...
def main() -> None:
    ...
    model = MultiStepRobotObjectMCGDFWorldModel(
        ...
        omit_object_context=cfg.omit_object_context,
        contact_flag_hidden_dim=cfg.contact_flag_hidden_dim,
        contact_flag_depth=cfg.contact_flag_depth,
        filter_steps=cfg.filter_steps,
        latent_update_hidden_dim=cfg.latent_update_hidden_dim,
        latent_update_depth=cfg.latent_update_depth,
    ).to(device)
