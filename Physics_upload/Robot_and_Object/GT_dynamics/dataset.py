...
EpisodeRef = Tuple[str, str]  # (hdf5_path, episode_name)


@dataclass
class RobotObjectGTDynamicsStateLayout:
    """State layout for hybrid robot-object GT-dynamics training."""
    ...
    def robot_dq_slice(self) -> slice:
        return slice(self.robot_dof, 2 * self.robot_dof)

    @property
    def object_pos_slice(self) -> slice:
        start = self.robot_state_dim
...
def first_object_motion_timestep(
    ...
    """Detect first object-motion/contact proxy from one episode."""
...
class RobotObjectGTDynamicsRolloutDataset(Dataset):
    """Rollout samples with robot DeLaN labels and object rigid-body labels."""
    ...
    def __init__(
        ...
        layout: RobotObjectGTDynamicsStateLayout | None = None,
    ) -> None:
        super().__init__()
        if history_len < 2:
        ...
        self.layout = layout or RobotObjectGTDynamicsStateLayout()
    ...
    def _material_context(self, object_group, t_count: int) -> np.ndarray:
        ...
        if material.shape[1] < self.layout.object_material_dim:
            pad = np.zeros((material.shape[0], self.layout.object_material_dim - material.shape[1]), dtype=np.float32)
    ...
    def _build_samples(
            ...
            with _h5_open(hdf5_path) as file:
                ...
                for episode_name in names:
                    ...
                    if any(key not in robot_dyn_group for key in self.robot_dynamics_keys):
                        self.skipped_missing_robot_dynamics_episodes += 1
                        continue
                    if any(key not in object_dyn_group for key in self.object_dynamics_keys):
                        self.skipped_missing_object_dynamics_episodes += 1
                        continue
                    ...
                    object_pos = root_pose[:, : self.layout.object_pos_dim]
                    ...
                    inertia = np.asarray(object_dyn_group["inertia"], dtype=np.float32).reshape(-1, self.layout.object_inertia_dim)
                    ...
                    object_context = np.concatenate([mass[:t_count], inertia[:t_count], material[:t_count]], axis=-1)
                    ...
                    for t in range(k, t_count - h):
                        ...
                        self.object_context.append(object_context[t])
        ...
        if len(self.history_states) == 0:
            raise RuntimeError("No valid robot-object GT-dynamics rollout samples were built.")
