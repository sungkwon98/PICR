"""Mean N-step prediction-error curves for the MCGDF world model.
...
# Heavy/light datasets live under the Physics_jj tree.  The MCGDF model
# itself is loaded from this folder's local ``outputs_mcgdf/`` by default.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to the trained MCGDF checkpoint (best.pt/last.pt).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output", type=str,
        default="./eval_outputs/mcgdf_pred_error_heavy_light.png",
...
def main() -> None:
    ...
    fig.suptitle(
        f"MCGDF open-loop {args.pred_horizon}-step prediction error  "
