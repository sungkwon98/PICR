...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Closed-loop filter window length for the latent online "
             "update.  -1 (default) means 'use the value baked into the "
             "checkpoint config'; 0 disables the filter; >0 overrides.",
    )
...
def rollout_open_loop(
    ...
    with torch.inference_mode():
        ...
        # Optional closed-loop filter window before the open-loop rollout.
        # Uses the last ``filter_steps`` observed transitions of the
        # history window to refine ``z`` via the latent innovation
        # update.  Reads only s_obs (joint encoders + object pose), never
        # GT xi, so it is deployment-fair.
        eff_filter = int(max(0, filter_steps))
        if eff_filter > 0:
            if z is None or model.latent_update is None:
                raise RuntimeError(
                    "filter_steps>0 requires a checkpoint trained with "
                    "use_context_encoder=True."
                )
            available = history_states.shape[1] - 1
            if eff_filter > available:
                raise ValueError(
                    f"filter_steps={eff_filter} exceeds available transitions "
                    f"({available}) in the history window; reduce filter_steps "
                    f"or increase history_len."
                )
            start = available - eff_filter
...
def main() -> None:
    ...
    # Filter steps: -1 → use checkpoint default; else override.
    if args.filter_steps < 0:
        filter_steps_eff = int(cfg.get("filter_steps", 0))
    else:
        filter_steps_eff = int(args.filter_steps)
